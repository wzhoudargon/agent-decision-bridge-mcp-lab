...
class ConnectedAgentConsultationPromptTests(unittest.TestCase):
    def test_default_prompt_uses_self_directed_project_inspection(self):
        ...
        self.assertIn("three internal permission modes, not three product tiers", text)
        self.assertIn("approval: the default", text)
        self.assertIn("controlled_auto", text)
        self.assertIn("file_info, then preview_patch", text)
        self.assertIn("call grant_action_approval", text)
